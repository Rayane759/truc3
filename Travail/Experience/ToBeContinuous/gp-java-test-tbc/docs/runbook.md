...
Ce document décrit les procédures opérationnelles du service `testtobecontinuous`.  
...
```bash
argocd app sync testtobecontinuous
```

### Redémarrer un pod

```bash
kubectl rollout restart deployment/testtobecontinuous -n <NAMESPACE>
```

### Vérifier l'état du service

```bash
kubectl get pods -n <NAMESPACE> -l app=testtobecontinuous
...
```bash
kubectl logs -f deployment/testtobecontinuous -n <NAMESPACE>
```
...
argocd app history testtobecontinuous

# Revenir à une révision précédente
argocd app rollback testtobecontinuous <REVISION>
```
