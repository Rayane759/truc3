# Runbook — java-test-CI-TBC

Ce document décrit les procédures opérationnelles du service `testci`.  
...
```bash
argocd app sync testci
```

### Redémarrer un pod

```bash
kubectl rollout restart deployment/testci -n <NAMESPACE>
```

### Vérifier l'état du service

```bash
kubectl get pods -n <NAMESPACE> -l app=testci
...
```bash
kubectl logs -f deployment/testci -n <NAMESPACE>
```
...
argocd app history testci

# Revenir à une révision précédente
argocd app rollback testci <REVISION>
```
