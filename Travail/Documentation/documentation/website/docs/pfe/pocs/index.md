...
**Répartition des responsabilités** : chaque équipe service (capability team) publie et reste responsable de sa propre brique — un Provider, une Composition sur son domaine (base de données, client Keycloak, bucket…) — comme une API réutilisable. L'équipe Platform Experience **assemble** ces briques en services de plus haut niveau (composition de compositions) et les distribue via le catalogue Backstage, sans en devenir responsable technique. IDDA, qui héberge temporairement les ressources DevExpérience (voir [ADR-003](../adr/adr-003-insertion-equipe.md)), configure à ce titre les Compositions et Providers du périmètre plateforme. Cette répartition évite de concentrer l'expertise de chaque domaine sur une seule équipe : un développeur peut aussi consommer une brique directement (Git / kubectl), sans passer par la plateforme.
...
Aucune expérimentation menée à ce jour sur ce périmètre — voir la [piste à explorer](./observabilite.md) qui pose le scope d'un futur POC.